"""The core typing module forming the base of the hyperTyping module"""
#  MIT Licence
...
def _check_generic(cls, parameters, elen):
  ...
  alen = len(parameters)
  if alen != elen:
    raise TypeError(f"Too {'many' if alen > elen else 'few'} arguments for "
                    f"{cls};"
...
def LiteralString(self, parameters):
  ...
  Example::
      ...
      def caller(arbitrary_string: str, literal_string: LiteralString) ->
      None:
...
def Concatenate(self, parameters):
  ...
  parameters = (
    *(_type_check(p, msg) for p in parameters[:-1]), parameters[-1])
...
class TypeVar(_Final, _Immutable, _BoundVarianceMixin, _PickleUsingNameMixin,
  ...
  def __typing_subst__(self, arg):
    ...
    if ((isinstance(arg, _GenericAlias) and arg.__origin__ is Unpack) or
        (isinstance(arg, GenericAlias) and getattr(arg,
                                                   '__unpacked__',
                                                   False))):
      raise TypeError(f"{arg} is not valid as type argument")
    return arg
...
class _BaseGenericAlias(_Final, _root=True):
  ...
  This represents a generic version of type 'origin' with type arguments
  'params'.
  There are two kind of these aliases: user defined and special. The
  special ones
  are wrappers around builtin collections and ABCs in collections.abc.
  These must
  ...
  def __dir__(self):
    return list(set(super().__dir__()
                    + [attr for attr in dir(self.__origin__) if
                       not _is_dunder(attr)]))
...
class _GenericAlias(_BaseGenericAlias, _root=True):
  ...
  def _make_substitution(self, args, new_arg_by_param):
    ...
    for old_arg in args:
      ...
      if self.__origin__ == collections.abc.Callable and isinstance(new_arg,
                                                                    tuple):
  ...
  def copy_with(self, args):
    return self.__class__(self.__origin__,
                          args,
                          name=self._name,
                          inst=self._inst,
...
class _SpecialGenericAlias(_NotIterable, _BaseGenericAlias, _root=True):
  def __init__(self, origin, nparams, *, inst=True, name=None):
    ...
    if origin.__module__ == 'builtins':
      ...
      self.__doc__ = (f'A generic version of {origin.__module__}.'
                      f'{origin.__qualname__}.')
...
_SPECIAL_NAMES = ['__abstractmethods__', '__annotations__', '__dict__',
                  '__doc__',
...
def _is_callable_members_only(cls):
  # PEP 544 prohibits using issubclass() with protocols that have
  # non-method members.
  return all(
...
def _no_init_or_replace_init(self, *args, **kwargs):
  ...
  # Initially, `__init__` of a protocol subclass is set to
  # `_no_init_or_replace_init`.
  # The first instantiation of the subclass will call
  # `_no_init_or_replace_init` which
  # searches for a proper new `__init__` in the MRO. The new `__init__`
  # replaces the subclass' old `__init__` (ie `_no_init_or_replace_init`).
  # Subsequent
...
class Annotated:
  ...
  - Nested Annotated are flattened::
      ...
      assert Annotated[Annotated[T, Ann1, Ann2], Ann3] == Annotated[T,
      Ann1, Ann2, Ann3]
...
def assert_type(val, typ, /):
  """Ask a static type checker to confirm that the value is of the given
  type.

  At runtime this does nothing: it returns the first argument unchanged
  with no
...
def get_args(tp):
  ...
  Examples::
      ...
      assert get_args(Union[int, Tuple[T, int]][str]) == (int, Tuple[str,
      int])
...
ContextManager = _alias(contextlib.AbstractContextManager,
                        1,
                        name='ContextManager')
AsyncContextManager = _alias(contextlib.AbstractAsyncContextManager,
                             1,
...
_prohibited = frozenset({'__new__', '__init__', '__slots__',
                         '__getnewargs__',
...
def TypedDict(typename, fields=None, /, *, total=True, **kwargs):
  ...
  individual keys as being required or not required::
      ...
      class Point2D(TypedDict):
          x: int               # the "x" key must always be present (
  ...
  def __mro_entries__(self, bases):
    ...
    class Dummy:
      def __init_subclass__(cls):
        ...
        raise TypeError(
          f"Cannot subclass an instance of NewType. Perhaps you were "
          f"looking for: "
          f"`{subclass_name} = NewType({subclass_name!r}, "
          f"{superclass_name})`"
        )
